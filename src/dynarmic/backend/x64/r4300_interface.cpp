...
#include "dynarmic/backend/x64/r4300_emit_x64.h"
#include "dynarmic/backend/x64/r4300_jitstate.h"
...
#include "dynarmic/interface/R4300/r4300.h"
...
static RunCodeCallbacks GenRunCodeCallbacks(R4300::UserCallbacks* cb, CodePtr (*LookupBlock)(void* lookup_block_arg), void* arg, const R4300::UserConfig& conf) {
    return RunCodeCallbacks{
        std::make_unique<ArgCallback>(LookupBlock, reinterpret_cast<u64>(arg)),
        std::make_unique<ArgCallback>(Devirtualize<&R4300::UserCallbacks::AddTicks>(cb)),
        std::make_unique<ArgCallback>(Devirtualize<&R4300::UserCallbacks::GetTicksRemaining>(cb)),
        conf.enable_cycle_counting,
    };
}

static std::function<void(BlockOfCode&)> GenRCP(const R4300::UserConfig& conf) {
...
public:
    ...
    HaltReason Run() {
        ...
        const CodePtr current_code_ptr = [this] {
            // RSB optimization
            const u32 new_rsb_ptr = (jit_state.rsb_ptr - 1) & R4300JitState::RSBPtrMask;
...
private:
    ...
    CodePtr GetCurrentSingleStep() {
        return GetBlock(R4300::LocationDescriptor{GetCurrentLocation()}.SetSingleStepping(true));
    }
    ...
    CodePtr GetBlock(IR::LocationDescriptor current_location) {
        ...
        IR::Block ir_block = R4300::Translate(R4300::LocationDescriptor{current_location}, get_code,
                                            {conf.define_unpredictable_behaviour, conf.wall_clock_cntpct});
        Optimization::PolyfillPass(ir_block, polyfill_options);
        Optimization::R4300CallbackConfigPass(ir_block, conf);
        Optimization::NamingPass(ir_block);
        if (conf.HasOptimization(OptimizationFlag::GetSetElimination) && !conf.check_halt_on_memory_access) {
            Optimization::R4300GetSetElimination(ir_block);
        ...
        if (conf.HasOptimization(OptimizationFlag::MiscIROpt)) {
            Optimization::R4300MergeInterpretBlocksPass(ir_block, conf.callbacks);
        }
    ...
    R4300JitState jit_state;
    BlockOfCode block_of_code;
    R4300EmitX64 emitter;
